...
// Component for Working Hours Modal
const WorkingHoursModal = ({ isOpen, onClose, availability }) => {
  if (!isOpen) return null;

  const modalRoot = document.getElementById("modal-root");
  if (!modalRoot) {
    console.error("Modal root element not found for WorkingHoursModal");
    return null;
  }

  return ReactDOM.createPortal(
    <>
      <div className="working-hours-modal-overlay" onClick={onClose}></div>
...
const DoctorProfile = () => {
  ...
  const editHandleSubmit = async (e) => {
    e.preventDefault();
    const doctorId = doctor.id; // Use the doctor user ID
